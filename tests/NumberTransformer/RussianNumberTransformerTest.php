...
class RussianNumberTransformerTest extends NumberTransformerTest
{
    ...
    public function providerItConvertsNumbersToWords()
    {
        return [
            ...
            [2342535345363436,'два квадриллиона триста сорок два триллиона пятьсот тридцать пять миллиардов триста сорок пять миллионов триста шестьдесят три тысячи четыреста тридцать шесть'],
            [5342535345363436,'пять квадриллионов триста сорок два триллиона пятьсот тридцать пять миллиардов триста сорок пять миллионов триста шестьдесят три тысячи четыреста тридцать шесть'],
            [2342535345365393436,'два секстиллиона триста сорок два квадриллиона пятьсот тридцать пять триллионов триста сорок пять миллиардов триста шестьдесят пять миллионов триста девяноста три тысячи четыреста тридцать шесть'],
            [5342535345365393436,'пять секстиллионов триста сорок два квадриллиона пятьсот тридцать пять триллионов триста сорок пять миллиардов триста шестьдесят пять миллионов триста девяноста три тысячи четыреста тридцать шесть'],
